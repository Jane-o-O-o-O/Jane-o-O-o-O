<div align="center">

![Jane-zz Banner](https://jane-zz.me/banner.png)

# 你好，我是 Jane-zz 👋

**AI Native Engineer / 全栈开发者**

专注于 **AI Agent、RAG、GraphRAG、大模型应用开发**。  
热爱构建智能化产品，用技术创造价值。

[![Website](https://img.shields.io/badge/Website-jane--zz.me-111827?style=flat-square&logo=vercel&logoColor=white)](https://jane-zz.me)
[![GitHub](https://img.shields.io/badge/GitHub-Jane--o--O--o--O-181717?style=flat-square&logo=github)](https://github.com/Jane-o-O-o-O)
[![Email](https://img.shields.io/badge/Email-i%40jane--zz.me-EA4335?style=flat-square&logo=gmail&logoColor=white)](mailto:i@jane-zz.me)

</div>

---

## 关于我

- 武汉纺织大学人工智能专业本科生，关注 AI Agent 和大模型应用工程化。
- 有 3 段实习经历，覆盖 GraphRAG 知识图谱、Agent 工作流编排、计算机视觉目标检测。
- 参与 SkillHub 生态建设，服务 1000+ 开发者，Skills 数量超过 5000+。
- 从后端到前端，从算法到工程，持续把 AI 能力落到可运行、可交付的产品里。

> 您目前看到的所有关于我的信息，都不是我的最终形态。  
> 我一定会走得更远。  
> 任何行业都值得被 Agent 重构一遍。

---

## 技术栈

<div align="center">

**编程语言**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)

**前端 / 后端**

![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)

**AI / Agent**

![AgentScope](https://img.shields.io/badge/AgentScope-111827?style=flat-square)
![RAG](https://img.shields.io/badge/RAG-7C3AED?style=flat-square)
![GraphRAG](https://img.shields.io/badge/GraphRAG-2563EB?style=flat-square)
![MCP](https://img.shields.io/badge/MCP-059669?style=flat-square)
![LLM Fine--tuning](https://img.shields.io/badge/LLM_Fine--tuning-DB2777?style=flat-square)

**数据 / 工具链**

![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![Neo4j](https://img.shields.io/badge/Neo4j-008CC1?style=flat-square&logo=neo4j&logoColor=white)
![Elasticsearch](https://img.shields.io/badge/Elasticsearch-005571?style=flat-square&logo=elasticsearch&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)

</div>

---

## 重点项目

### [SSHFerry](https://github.com/Jane-o-O-o-O/SSHFerry)

多会话 SSH 文件传输工作区，PySide6 桌面客户端 + FastAPI 后端。支持上传、下载、远端互传、`remote_root` 沙箱安全边界和任务可视化管控。

`Python` `PySide6` `FastAPI` `SSH` `SFTP`  
[Live Demo](https://sshferry.cloud) · [GitHub](https://github.com/Jane-o-O-o-O/SSHFerry)

### [RelationGraph](https://github.com/Jane-o-O-o-O/RelationGraph)

Neo4j + FastAPI + React 的 3D 知识图谱可视化，支持 spaCy 实体抽取和 60+ 别名消歧规则。

`Neo4j` `React` `FastAPI` `spaCy` `3D`  
[Live Demo](https://graph.jane-zz.me) · [GitHub](https://github.com/Jane-o-O-o-O/RelationGraph)

### [Agent-Pulse-Skill](https://github.com/Jane-o-O-o-O/agent-pulse-skill)

Agent Pulse 集成扩展，skills.sh 下载安装量 4.7k。基于 Codex Skill 机制封装本地 AI Agent 会话分析能力，支持日志检索、Token 统计、模型成本估算、预算预测、健康检查和报表导出。

`Codex Skill` `Agent` `CLI` `Analytics` `skills.sh`  
[skills.sh](https://www.skills.sh/jane-o-o-o-o/agent-pulse-skills/agent-pulse) · [GitHub](https://github.com/Jane-o-O-o-O/agent-pulse-skill)

### [OpenChat](https://chat.jane-zz.me)

大模型对话平台，支持系统提示词、工具调用、流式响应、温度、Top-p、Top-k、`max_tokens`、停止序列、惩罚参数、seed 和 reasoning effort 等核心参数配置。

`LLM` `Chat` `Tools` `Streaming` `Prompt`  
[Open](https://chat.jane-zz.me)

---

## 实习经历

**全栈工程实习生 · 北京未来式智能科技有限公司**  
`2026.02 - 2026.05`

- 参与 GraphRAG 模块升级，覆盖无向图建模、Leiden 社区发现、社区摘要全链路。
- 参与 Neo4j / Elasticsearch 持久化和本地 NLP 模型增量更新，降低图谱构建成本。
- 参与 SkillHub 生态建设，服务 1000+ 开发者，Skills 5000+。

**Agent 研发实习生 · 武汉绘梦心河有限公司**  
`2025.09 - 2025.12`

- 从零搭建 AgentScope 论文生成工作流，设计主 Agent + 子 Agent 协作流程。
- 参与 Agent 工具调用、MCP、Skills 开发。
- 使用 React + FastAPI 做全栈功能开发。

**研究助理 · 中国船舶集团第七〇一研究所**  
`2024.09 - 2025.03`

- 参与双模态水上目标识别方法研发与验证。
- 完成与 YOLOv8 / FusionVIRNet 的对比实验。
- 参与目标检测技术和大模型微调优化。

---

## 奖项

- 全国大学生测绘程序设计大赛：全国特等奖，前 5%，2024。
- China Robot Competition & RoboCup：全国三等奖，2024。
- 全球算法精英大赛 · 无人车任务挑战赛：国家级获奖，2025.12。
- 省级创新创业项目。

---

## GitHub 数据

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=Jane-o-O-o-O&show_icons=true&theme=transparent&hide_border=true" width="48%" alt="GitHub Stats" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=Jane-o-O-o-O&theme=transparent&hide_border=true" width="48%" alt="GitHub Streak" />

![Profile Views](https://komarev.com/ghpvc/?username=Jane-o-O-o-O&color=2563eb&style=flat-square)

</div>
